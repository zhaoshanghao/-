...
%
%  FACE_NODE may contain polygons of different orders.FACE_NODE可能包含不同顺序的多边形
%  To make the call to PATCH, we will assume all the polygons are the same order.在调用PAICH时，我们假设所有的多边形都是相同的顺序。
%  To do so, we'll simply "stutter" the first node in each face list.为了做到这一点，我们将简单地“结巴”在每个面孔列表中的第一个节点。
%
  for face = 1 : face_num
...
%
%  显示形状
%  The TITLE function will interpret underscores in the title.TITLE函数将解释标题中的下划线
%  We need to unescape such escape sequences!我们需要避开这样的转义序列
  return
end
function c = ch_cap ( c )
